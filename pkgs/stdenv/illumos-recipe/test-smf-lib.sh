#!/usr/bin/env bash
#
# Test smf-lib.nix end-to-end on a live illumos zone:
#
#   1. zone-root/services.nix evaluates and the generated nix-daemon
#      manifest + bundle build.
#   2. `svccfg validate` accepts every generated manifest.
#   3. Golden parity: the hand-written zone-root/nix-daemon.xml and the
...
if gen_daemon=$(nix-build --no-out-link "$recipe/zone-root/services.nix" -A nix-daemon 2>"$tmp/build.err"); then
...
if bundle=$(nix-build --no-out-link "$recipe/zone-root/services.nix" -A bundle 2>"$tmp/build.err"); then
...
if export_one "$recipe/zone-root/nix-daemon.xml" "$tmp/golden.db" >"$tmp/golden.export" \
...
let
  pkgs = import $recipe/../../.. { };
  smf = import $recipe/smf-lib.nix { inherit pkgs; };
