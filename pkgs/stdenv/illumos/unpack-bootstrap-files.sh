...
# gcc's built-in specs name its runtime library directory, now in the padded form. A specs file is text and
# takes precedence, so give gcc one with the plain path: the ld wrapper's purity check does not know
# ////nix/store/... as a store path, and the programs built get a RUNPATH without the padding.
gccdir=$(dirname "$(gcc -print-libgcc-file-name)")
gcc -dumpspecs | sed -e 's|//*nix/store/|/nix/store/|g' > "$gccdir/specs" || exit 1
