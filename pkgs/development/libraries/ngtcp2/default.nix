...
stdenv.mkDerivation (finalAttrs: {
  ...
  cmakeFlags = [
    ...
    # Also, the examples depend on Linux-specific APIs (like IPTOS_ECN_MASK), so we avoid them on FreeBSD and illumos too.
    (lib.cmakeBool "ENABLE_LIB_ONLY" (stdenv.hostPlatform.isStatic || stdenv.hostPlatform.isFreeBSD || stdenv.hostPlatform.isIllumos))
